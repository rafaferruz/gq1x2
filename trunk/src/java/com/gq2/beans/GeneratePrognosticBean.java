 ...
 */
@ManagedBean(name = "genPrognostic")
@ViewScoped
public class GeneratePrognosticBean {
    ...
    /**
     * Creates a new instance of fgenerarPrognosticBean
     */
    public GeneratePrognosticBean() {
    }

    public GeneratePrognosticBean(int chaId, int round) {
	this.chaId = chaId;
	this.round = round;
    }
	...
	MakePrognostic makePrognostic = new MakePrognostic();

	switch (generateMode) {
	    case 0: // Se procesa solamente la ronda seleccionada de un campeonato
		makePrognostic.processRound(chaId, round);
		break;
		...
		for (SelectItem item : roundItemList.subList(1, roundItemList.size())) {
			...
			makePrognostic.processRound(chaId, (Integer) item.getValue());
		    }
		}
		break;
		...
		System.out.println("Generaci�n Prognostic finalizada. Campeonato "
