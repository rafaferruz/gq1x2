...
import com.gq2.services.MakePrePool;
 ...
 */
@ManagedBean(name = "genPrePool")
@ViewScoped
public class GeneratePrePoolBean {
    ...
    /**
     * Creates a new instance of fgenerarPrePoolBean
     */
    public GeneratePrePoolBean() {
    }

    public GeneratePrePoolBean(int chaId, int round) {
	this.chaId = chaId;
	this.round = round;
    }
	...
	MakePrePool makePrePool = new MakePrePool();

	switch (generateMode) {
	    case 0: // Se procesa solamente la ronda seleccionada de un campeonato
		makePrePool.processRound(chaId, round);
		break;
		...
		for (SelectItem item : roundItemList.subList(1, roundItemList.size())) {
			...
			makePrePool.processRound(chaId, (Integer) item.getValue());
		    }
		}
		break;
		...
		System.out.println("Generaci�n PrePool finalizada. Campeonato "
